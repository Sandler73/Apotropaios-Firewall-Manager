...
_backup_export_single() {
    ...
    case "${fw_name}" in
        ...
        ufw)
            if util_is_command_available ufw; then
                ...
                # Also copy ufw config files if accessible
                [[ -d /etc/ufw ]] && cp -r /etc/ufw "${staging}/ufw_etc/" 2>/dev/null || true
            fi
            ;;
        ipset)
    ...
    [[ -f "${output_file}" ]] && chmod "${SECURE_FILE_PERMS}" "${output_file}" 2>/dev/null || true
}
